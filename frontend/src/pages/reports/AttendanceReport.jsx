...
export function AttendanceReport() {
  ...
  const handleExport = () => {
    ...
    if (department) filters.department = department;
    if (dateRange) filters.dateRange = dateRange;
    reportService.exportAttendance(filters);
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <PageHeader
          ...
          description="Daily check-ins, worked hours, and statuses."
        />
        <button
              ...
              {loading ? (
                ...
                filteredData.map((record) => (
                  <tr key={record.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{record.workedHours}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <span className={cn("inline-flex items-center px-2 py-0.5 rounded text-xs font-medium", 
                        record.status === 'Present' ? 'bg-green-100 text-green-800' : 
                        record.status === 'Late' ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800'
                      )}>
                        {record.status}
                      </span>
                    </td>
